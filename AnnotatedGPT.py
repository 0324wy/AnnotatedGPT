...
def check_outputs(
    ...
    for idx in range(n_examples):
        ...
        rb = Batch(b[0], b[1])
        greedy_decode(model, rb.src, 64)[0]

        src_tokens = [vocab.get_itos()[x] for x in rb.src[0]]
        tgt_tokens = [vocab.get_itos()[x] for x in rb.tgt[0]]
        ...
        model_out = greedy_decode(model, rb.src, 64)[0]
        model_txt = (
                " ".join(
                    [vocab.get_itos()[x] for x in model_out]
...
def run_model_example(n_examples=5):
    ...
    model.load_state_dict(
        torch.load("cet_final.pt", map_location=torch.device("cpu"))
    )

    print("Checking Model Outputs:")
    example_data = check_outputs(
        valid_dataloader, model, vocab, n_examples=n_examples
    )
